...
In pi, the role-based reviewers in `agents/` (loaded from `~/.pi/agent/agents/` or project `.pi/agents/`) start with isolated context by design and are usable here — see `.pi/agents/` for the roster and per-domain match.
